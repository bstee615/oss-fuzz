...
python3 infra/helper.py reproduce --num_runs 1 --container_name foo $PROJECT $FUZZER $CORPUS_INPUT timeout=300 instrumentation_excludes="**" || (echo "$PROJECT $FUZZER reproduce-INSTRUMENT" >> $FAILURE_LOG_FILE && exit 1)
